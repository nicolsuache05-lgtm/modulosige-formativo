...
@section('title', 'Editar Rol - Administración')
    ...
    <div class="col-lg-9">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4 pb-2 border-bottom">
            <div>
                ...
                <h2 class="fw-bold text-dark mb-0">Modificar Rol: {{ $role->name }}</h2>
                <p class="text-muted fs-6 mb-0">Actualización de datos del perfil institucional y matriz de permisos.</p>
            </div>
        </div>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="full_access" class="form-select bg-light" required>
                            <option value="No" {{ old('full_access', $role->full_access) == 'No' ? 'selected' : '' }}>No (Permisos granulares)</option>
                            <option value="Si" {{ old('full_access', $role->full_access) == 'Si' ? 'selected' : '' }}>Si (Acceso irrestricto)</option>
                        </select>
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="description" rows="2" class="form-control bg-light">{{ old('description', $role->description) }}</textarea>
                    </div>
                                ...
                                @forelse($permisos as $p)
                                    <div class="col-md-6">
                                        <div class="form-check p-2 bg-white rounded-3 border">
                                            <input class="form-check-input ms-1" type="checkbox" name="permissions[]" value="{{ $p->id }}" id="perm_{{ $p->id }}" {{ in_array($p->id, old('permissions', $rolePermIds)) ? 'checked' : '' }}>
                    ...
                    <div class="col-12 d-flex justify-content-end gap-2 pt-3 border-top">
                        ...
                        <button type="submit" class="btn btn-admin px-5">
                            <i class="fas fa-sync-alt me-1"></i> Actualizar Rol
                        </button>
                    </div>
                </div>
            </form>
